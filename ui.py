...
def common(req=None,pfx='.',edit=False,user='',msg=''):
    ...
    o += defs()
    if edit:
        o += '<foreignObject id=".editor" display="none" y="20" width="400px" height="400px"><div %s id="editor">%s</div></foreignObject>'%(_XHTMLNS,value)
    mygraph = cg(value)
    mygraph.set_pos()
    o += mygraph.draw()    
    if edit:
...
def menu():
    """ """
    o = '<g id=".menu"><rect fill="#CCC"/><text class="small">'
    o += '<tspan>Ace</tspan>'
    o += '<tspan dy="1em" x="0">Goal</tspan>'
    o += '<tspan dy="1em" x="0">Requirement</tspan>'
    o += '<tspan dy="1em" x="0">Agent</tspan>'
...
def login_logout(user,msg):
    ...
    o += '<text onclick="%s();" text-anchor="end" x="99%%" y="12" class="button">%s<title>%s</title></text>'%(action,txt,action)
    color = 'red' if msg[:5] == 'Error' else 'green'
    o += '<text id=".msg" fill="%s" x="10" y="99%%">%s</text>'%(color,msg)
    return o + '</g>\n'
...
def login_page(req):
    ...
    o += '<foreignObject display="inline" y="200" x="200" width="80" height="70">' 
    ...
    o += '<g onclick="check();" title="submit login/password" class="button" fill="#444" transform="translate(280,208)"><rect x="1" width="15" height="30" rx="5"/><path transform="translate(0,6)" d="M4,4 4,14 14,9" fill="white"/></g>'
