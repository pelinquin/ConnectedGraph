...
def cg_parent(raw,idc):
    ...
    if ch.has_key(idc):
        ...
        if typ.has_key(ch[idc]):
            role = typ[ch[idc]]
    o = '<text id=".noderole" x="18" y="50" class="noderole">%s </text>\n'%role
    o += '<text id=".nodelabel" class="header" x="72" y="50">%s </text>\n'%emphasis(label)
    return o     
    ...
    def graph(self,head=False):
        ...
        for i in self.lab.keys():
            ...
            if self.child.has_key(i):
                ...
                if self.rev and mygit.test(self.child[i],self.rev): # attachdoc 
                    ref += ' attach="yes"'
        ...
        o += '<text id=".stat" class="stat" x="8" dy="-8" y="100%%">%d nodes %d connectors<title>Statistics</title></text>'%(len(self.lab.keys()),len(self.connectors))
        return o + '</g>\n'
...
def mode_button(mG,mT):
    ...
    o += '<g id=".rmode" stroke="white" display="%s"><title>viewer mode</title><rect width="300" height="290" rx="50"/>'%mT
...
def basic(req=None,edit=False,mode='graph',valGet='',pfx='..',user='',msg=''):
    ...
    o += '<text id=".content" class="hd" y="0">%s<title>diagram content</title></text>'%short(content)
    o += '<text id=".gid" class="hd" y="10">%s<title>diagram id</title></text>'%gid
    ...
    if edit:
        if user != 'anonymous':
            o += '<text id=".user" class="hd1" x="470" y="12" onclick="logout();">%s<title>logout</title></text>'%user
        else:
            o += '<text id=".user" display="none">anonymous</text>' # revoir
        ...
        o += '<text id=".ip" class="hd1" text-anchor="end" x="100%%" y="22">%s<title>ip address</title></text>'%ip
        ...
        o += '</g>'
        o += '<text id=".status" class="hd1" title="status" x="640" y="12" fill="#999">%s</text>'%msg
